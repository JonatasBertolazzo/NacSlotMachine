...
public class SlotMachineActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (masq){
            background.setBackgroundResource(R.drawable.wallpaper_snake);
        }

        if (femin){
            background.setBackgroundResource(R.drawable.wallpaper_eva);
        }
    ...
    public void jogar(View v){
        ...
        if(fichas > 0){
            ...
            Toast.makeText(this, "Acabaram suas fichas D:", Toast.LENGTH_LONG).show();
        }
    ...
    private void exibeResultado(){
        if(slot1.indiceAtual == slot2.indiceAtual && slot2.indiceAtual == slot3.indiceAtual){
            Toast.makeText(this, "Voce ganhou! +2 Fichas! :D", Toast.LENGTH_LONG).show();
            fichas+=2;
        } else if(slot1.indiceAtual == slot2.indiceAtual || slot2.indiceAtual == slot3.indiceAtual || slot1.indiceAtual == slot3.indiceAtual){
            Toast.makeText(this, "Pequena premiação! +1 Ficha! :)", Toast.LENGTH_LONG).show();
            fichas+=1;
        } else {
            Toast.makeText(this, "Voce perdeu! -1 Ficha :(", Toast.LENGTH_LONG).show();
        }
